## Hi there 👋

I'm Gavin Wang, a student developer building educational software, campus tools, and social-impact technology.

I work on projects around learning, accessibility, writing workflows, and community infrastructure. I’m especially interested in how software can support education, public-good systems, juvenile rehabilitation, and reentry.

- Pronouns: he/him
- Languages: Shanghainese · Mandarin · English · Japanese

<p>
  <img alt="Python" src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img alt="TypeScript" src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" />
  <img alt="Go" src="https://img.shields.io/badge/Go-00ADD8?style=flat-square&logo=go&logoColor=white" />
  <img alt="Emacs" src="https://img.shields.io/badge/Emacs-7F5AB6?style=flat-square&logo=gnuemacs&logoColor=white" />
  <img alt="Lisp" src="https://img.shields.io/badge/Lisp-3fb68b?style=flat-square" />
  <img alt="Swift" src="https://img.shields.io/badge/Swift-F05138?style=flat-square&logo=swift&logoColor=white" />
  <img alt="Git" src="https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white" />
  <img alt="Bun" src="https://img.shields.io/badge/Bun-000000?style=flat-square&logo=bun&logoColor=white" />
  <img alt="Svelte" src="https://img.shields.io/badge/Svelte-FF3E00?style=flat-square&logo=svelte&logoColor=white" />
</p>

## Focus

- Educational software and assessment tools
- Campus apps and student-facing infrastructure
- Accessibility and social-impact technology
- Local-first tools, writing workflows, and community publishing
- Computational experiments inspired by mathematics, physics, and programming languages

## My work

- [`GWen`](https://github.com/Gavin-WangSC/GWen) — an IB Chinese A SL AI grading assistant for essay assessment
- [`Outspire Android`](https://github.com/Computerization/Outspire-Android) — a native Android port of Outspire built against the TSIMS backend, with login, timetable, scores, and CAS support
- [`Outspire`](https://github.com/Computerization/Outspire) — an all-in-one WFLA campus companion for iOS and macOS, covering schedules, academics, CAS, school life, widgets, and deep links
- [`WiseLens`](https://github.com/Computerization/media-filter) — an information verification assistant designed to help elderly users identify fake news and misleading online content
- [`Prologue`](https://github.com/Computerization/Prologue) — a WeChat Mini Program project under Computerization, with mini-program code and cloud functions
- [`c13n`](https://github.com/Gavin-WangSC/c13n) — Computerization's newsletter and publishing workflow, with an Astro web version and TeX/PDF generation
- [`computerization.github.io`](https://github.com/Gavin-WangSC/computerization.github.io) — the public website for Computerization and its projects

## Social impact

I’m interested in juvenile rehabilitation, reentry, and the role of education in helping people return to society.

I also care about technology accessibility: tools should be understandable, usable, and useful to people outside purely technical communities.

## Contact

- GitHub: [@Gavin-WangSC](https://github.com/Gavin-WangSC)
- Email: [wangsc1124@gmail.com](mailto:wangsc1124@gmail.com)
- Email: [wangsc1124@163.com](mailto:wangsc1124@163.com)
